...
def get_nvidia_model():
    ...
    model.add(Lambda(lambda x: x / 127.5 - 1.0,
                     input_shape=shape,
                     output_shape=shape))
    model.add(Convolution2D(24, 5, 5, subsample=(2, 2), border_mode='valid', input_shape=shape, activation="relu"))
    model.add(ELU())
...
def get_model():
    ...
    model.add(Lambda(lambda x: x / 127.5 - 1.,
                     input_shape=shape,
                     output_shape=shape))
    model.add(Convolution2D(3, 1, 1, border_mode="same", activation="relu", input_shape=shape))
    model.add(MaxPooling2D((2, 2), border_mode='valid'))
